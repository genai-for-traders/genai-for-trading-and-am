...
class VAEModule(BaseLightningModule):
    ...
    def _compute_reconstruction_loss(
        self, x: torch.Tensor, reconstructed_x: torch.Tensor
    ) -> torch.Tensor:
    ...
    def step(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        loss, rec_loss, kl_loss = self.compute_loss(
            x, reconstructed_x, z_mean, z_log_var
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_kl_loss", kl_loss.item(), on_step=True, on_epoch=True, prog_bar=True
        )
        return loss
