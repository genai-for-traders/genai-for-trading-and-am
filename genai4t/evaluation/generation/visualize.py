...
def visualize_time_series(real_data: np.ndarray, synt_data: np.ndarray):
    """
    shape: [seq, n_feat]
    """
...
def visualize_2d_pca(real_data: np.ndarray, synt_data: np.ndarray, title: str = None):
    ...
    sns.scatterplot(x="P1", y="P2", data=pd_pca.query("real == 1"), ax=ax, label="real")
    ax.set_title(title)
    plt.legend()
...
def visualize_2d_tsne(real_data: np.ndarray, synt_data: np.ndarray, title: str = None):
    title = title or "TSNE"
    test_and_synt_data, is_real = create_real_synt_xy(real_data, synt_data)
    ...
    sns.scatterplot(
        x="P1", y="P2", data=pd_tsne.query("real == 1"), ax=ax, label="real"
    )
    ax.set_title(title)
    plt.legend()
    plot_style.apply_plot_style(ax)
