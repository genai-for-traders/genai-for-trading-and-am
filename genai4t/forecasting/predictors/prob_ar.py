...
class ProbabilisticNeuralARModel(BaseLightningModule):
    ...
    def get_parameters(self, past_target: torch.Tensor) -> DistributionOutput:
        ...
        state: torch.Tensor = self.mlp(scaled_past_target).view(
            -1, self.prediction_length, self.hidden_dim
        )
