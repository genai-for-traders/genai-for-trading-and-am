...
def apply_plot_style(ax, boxed=True, tight_layout=True):
    ...
    """
    ax.set_xlabel(
        ax.get_xlabel(), fontsize=LABEL_SIZE, fontname=FONT_FAMILY, fontweight="normal"
    )
    ax.set_ylabel(
        ax.get_ylabel(), fontsize=LABEL_SIZE, fontname=FONT_FAMILY, fontweight="normal"
    )
    ax.set_title(
        ax.get_title(), fontsize=TITLE_SIZE, fontname=FONT_FAMILY, fontweight="normal"
    )
